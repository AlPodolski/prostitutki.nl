...
class RknCommand extends Command
{
    ...
    public function handle(): void
    {
        ...
        $cityList = City::with('info')->get();

        $domains = array();

        foreach ($cityList as $city) {
            $domains[] = $city->info->domain;
        }
    ...
    private function addNewBlock($cityUrl, $blockCityItem, $newCityUrl)
    {
        ...
        $cityInfo = City::where(['url' => $cityUrl])->with('info')->first();

        $oldCity = $blockCityItem.'.'.$cityInfo->info->domain;
        ...
        if (!CityBlock::where(['old_city' => $oldCity])->first()) {
            ...
            $newCityBlock->site_id = $cityInfo->info->site_id;
                ...
                foreach ($domains as $domain) {
                    ...
                    if (strpos($value, $domain)) {
                        ...
                        foreach ($cityList as $cityItem) {
                            ...
                            if ($cityItem->info->actual_city) {

                                $tempName = $cityItem->info->actual_city . '.' . $cityItem->info->domain;

                                if ($value == $tempName) {

                                    $blockDomains['city'][] = $cityItem->info->actual_city ;
